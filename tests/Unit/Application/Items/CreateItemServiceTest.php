...
class CreateItemServiceTest extends TestCase
{
    ...
    {
        $data = ['tenant_id' => $this->tenant->id, 'name' => 'Test Coin', 'type' => 'coin', 'quantity' => 1, 'status' => 'in_collection'];
        $item = $this->service->handle($data);

        $this->assertInstanceOf(Item::class, $item);
        $this->assertDatabaseHas('items', ['id' => $item->id, 'name' => 'Test Coin', 'type' => 'coin']);
    }
    ...
    public function it_creates_an_item_and_syncs_its_text_attributes(): void
    {
        ...
        $data = [
            ...
            'type' => 'coin',
            'attributes' => [$attribute->id => ['value' => 'Silver']],
        ];
        $item = $this->service->handle($data);
    ...
    public function it_creates_an_item_and_syncs_its_select_attributes(): void
    {
        ...
        $data = [
            ...
            'name' => 'Graded Coin',
            'type' => 'coin',
            'attributes' => [$attribute->id => ['attribute_option_id' => $attributeValue->id]],
        ];
        $item = $this->service->handle($data);

        $this->assertDatabaseHas('item_attribute', ['item_id' => $item->id, 'shared_attribute_id' => $attribute->id, 'attribute_option_id' => $attributeValue->id, 'value' => 'UNC']);
    }
}
