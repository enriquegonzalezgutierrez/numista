...
class PublicItemController extends Controller
{
    ...
    {
        // The controller now only cleans up empty filter values before passing them to the finder.
        $filters = $request->all();
        ...
        // This part is crucial for our new AJAX filtering.
        ...
        $categories = Category::query()->whereHas('items', fn ($q) => $q->where('status', 'for_sale'))->orderBy('name')->get();
        $filterableAttributes = SharedAttribute::query()->where('is_filterable', true)->with('options')->orderBy('name')->get();
    ...
    public function show(Item $item): View
    {
        ...
        $item->load(['images', 'tenant', 'categories', 'customAttributes.options']);
